...
# Let's try a different approach with a cleaner visualization
# First, ensure the months are in the correct order
month_levels <- c("October 2022", "November 2022", "January 2023", "February 2023", 
                 "March 2023", "April 2023", "May 2023", "June 2023", 
                 "September 2023", "October 2023", "November 2023")
pcoa_data$Month_Year <- ifelse(pcoa_data$Month %in% c("October", "November") & 
                              as.numeric(as.factor(pcoa_data$Week)) <= 2,
                              paste(pcoa_data$Month, "2022"),
                              paste(pcoa_data$Month, "2023"))

pcoa_data$Month_Year <- factor(pcoa_data$Month_Year, levels = month_levels)
# Create a more focused plot
p_temporal_clean <- ggplot(pcoa_data, aes(x=Month_Year, y=PCo1, color=Individual, group=Individual)) +
  geom_line(linewidth=1, alpha=0.7) +
  geom_point(size=3) +
  theme_bw() +
  labs(x="", 
       y=paste0("PCo1 (", round(variance_explained[1], 1), "%)"),
...
monthly_stats <- aggregate(PCo1 ~ Month_Year + Individual, data=pcoa_data,
                         FUN=function(x) c(mean=mean(x), sd=sd(x), n=length(x)))

# Convert the results to a more readable format
monthly_stats_df <- data.frame(
  Month_Year = monthly_stats$Month_Year,
