...
```{r}
setwd("C:/Users/Owner/Desktop/Project-Howler/humaan")
path_data <- read.table("pathabundance-cmp.tsv" , header = TRUE , sep = '\t')
...
p1 <- ggplot(mean_abundance, aes(x = reorder(Pathway, mean_abundance), y = mean_abundance)) +
...
p2 <- ggplot(tidy_data %>% filter(Pathway %in% top5_genera), 
             aes(x = Season, y = Relative_Abundance, fill = Season)) +
  geom_boxplot() +
...
p1 <- ggplot(core_genera_filtered, 
       aes(x = reorder(Pathway, Mean_Abundance), 
           y = Mean_Abundance, 
           fill = Individual)) +
...
p2 <- ggplot(core_genera_filtered, 
       aes(x = reorder(Pathway, -Prevalence), 
           y = Individual, 
           fill = Prevalence)) +
  geom_tile() +
  scale_fill_viridis() +
...
p3 <- ggplot(core_genera_filtered, 
       aes(x = Individual, 
           y = reorder(Pathway, Mean_Abundance),
           size = Mean_Abundance,
           color = Prevalence)) +
...
p4 <- ggplot(tidy_data %>% 
               inner_join(core_genera_filtered %>% select(Individual, Pathway)), 
       aes(x = reorder(Pathway, Relative_Abundance), 
...
top10_genera <- mean_abundance$Pathway[1:10]
filtered_data <- tidy_data %>%
  filter(Pathway %in% top10_genera)
top10_genera
# Create a pivot table for the heatmap
heatmap_data <- filtered_data %>%
  select(SampleID, Pathway, Abundance) %>%
  spread(key = Pathway, value = Abundance, fill = 0) %>%
...
p1 <- pheatmap(
  ...
  show_rownames = TRUE,
  show_colnames = TRUE,
  main = "Heatmap of Top 10 Most Abundant PW"
)
p1
ggsave("Heatmap of Top 10 Most Abundant PW.jpg", plot = p1, width = 10, height = 8, dpi = 300)
...
# write.table(pcoa_data , "pcoa_data_genus.csv", sep=",", row.names = TRUE, col.names = TRUE)
...
head(agg_data_pw, 15)
# write.table(aggregated_data , "aggregated_pco1_data.csv", sep=",", row.names = TRUE, col.names = TRUE)
...
ggsave("Temporal_mean_magnitude_PW.jpg", plot = temporal_p_pw, width = 10, height = 8, dpi = 300)
```
