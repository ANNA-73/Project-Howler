...
# Define palettes globally (outside the function)
ind_palette <- c(
  "Isa"      = "#4F81BD",  # Blue
  "Uno"      = "#C0504D",  # Red
  "Dos"      = "#9BBB59",  # Green
...
# ============================================================
# KEY STEP: Create unique labels by combining with parent phylum
# ============================================================

bin_taxa_sankey <- bin_taxa_sankey %>%
  mutate(
    # Create unique identifiers for sorting purposes
    family_unique = paste0(phylum, "_", family),
    genus_unique = paste0(phylum, "_", genus)
  )

# Create ordering based on unique identifiers
family_order <- bin_taxa_sankey %>%
  group_by(phylum, family_unique) %>%
  summarise(n = n(), .groups = 'drop') %>%
  arrange(phylum, desc(n)) %>%
  pull(family_unique)

genus_order <- bin_taxa_sankey %>%
  group_by(phylum, genus_unique) %>%
  summarise(n = n(), .groups = 'drop') %>%
  arrange(phylum, desc(n)) %>%
  pull(genus_unique)
...
bin_taxa_sankey <- bin_taxa_sankey %>%
  mutate(
    family_unique = factor(family_unique, levels = family_order),
    genus_unique = factor(genus_unique, levels = genus_order)
  )

# ============================================================
# Option 1: Use unique identifiers in the plot (shows phylum context)
# ============================================================

sankey_simplified <- bin_taxa_sankey %>%
  select(phylum, family_unique, genus_unique) %>%
  make_long(phylum, family_unique, genus_unique)

p_sankey_2 <- ggplot(sankey_simplified, aes(
  x = x, 
  next_x = next_x, 
  node = node, 
  next_node = next_node,
  ...
  geom_sankey_label(
    # Remove the phylum prefix for cleaner labels
    aes(label = str_remove(node, "^[^_]+_")),
    size = 3.5, 
    color = "white", 
    fill = "gray40"
  ) +
...
# ============================================================
# For the version with samples:
# ============================================================

sankey_sample_all <- bin_taxa_sankey %>%
  select(sample, phylum, family_unique, genus_unique) %>%
  make_long(sample, phylum, family_unique, genus_unique)

p_sankey_3 <- ggplot(sankey_sample_all, aes(
  x = x,
  next_x = next_x,
  node = node,
  next_node = next_node,
  ...
  geom_sankey_label(
    data = function(d) {
      d %>% 
        dplyr::filter(x != "sample") %>%
        mutate(
          # Only remove prefix for family and genus, NOT for phylum
          clean_label = if_else(
            x %in% c("family_unique", "genus_unique"),
            str_remove(node, "^[^_]+_"),
            node
          )
        )
...
ggsave(
  file.path(plot_dir, "MAGs_sankey_order.jpg"),
  plot = p_sankey_3,
  width = 22,
  height = 18,
  dpi = 300
